defmodule ProjectC.Argx.General do
  @moduledoc false
...
defmodule ProjectC do
  ...
  use Argx.Matcher, ProjectC.Argx.General
...
defmodule ArgxMatcherTest do
  ...
  describe "no general defconfigs" do
    test "ok" do
      assert :ok == ProjectC.get(%{one: "one", two: 2, three: 3.3})
      assert :ok == ProjectC.get(one: "one", two: 2, three: 3.3)
    end

    test "error" do
      assert :error == ProjectC.get(%{one: "one", two: 2})
      assert :error == ProjectC.get(one: "one", three: 3.3)
    end
  end

  describe "mixed general defconfigs & defconfigs in current module" do
    test "ok" do
      assert :ok == ProjectC.post(%{one: "one", c: [1, 2], d: %{a: 1}})
      assert :ok == ProjectC.post(one: "one", c: [1, 2], d: %{a: 1})
    end

    test "error" do
      assert :error == ProjectC.post(%{one: "one", c: [1, 2]})
      assert :error == ProjectC.post(one: "one", c: true, d: %{a: 1})
    end
  end
end
